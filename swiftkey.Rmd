...
#testvector <- c("Go on a romantic date at the grocery", "romantic date at the beach", "romantic date at the movies", "romantic date at the mall")
...
findprob<-function(testvector) {
  for(i in 1:length(testvector)) {
    ...
    for(j in 2:length(ans1)-1) {
      ...
      finalprob[j-1] <- res1/sum(blg_uni[blg_uni$word==ans1[j],]$n, news_uni[news_uni$word==ans1[j],]$n, twit_uni[twit_uni$word==ans1[j],]$n)
...
testvector <- c("Go on a romantic date at the grocery", "Go on a romantic date at the beach")
